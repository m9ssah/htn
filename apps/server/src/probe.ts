...
const deps: TurnDeps = {
  // `buildGraph` replaced `createGraph` in the merge; the composer is what
  // lets generic_answer/message_drafts compose rather than halt, and it needs
  // the concrete HTTP client, so it is only wired when Jev is live.
  graph: buildGraph({ session, ...(jev === stubJevClient ? {} : { composer: liveComposer(getRealJevClient()) }) }),
  jev,
...
for (const utterance of utterances) {
  ...
  console.log(`  bowl        : ${session.task ? Object.entries(session.task.inBowl).map(([k, v]) => `${k}=${v}`).join(' ') || '(empty)' : '(nothing chosen yet)'}`);
